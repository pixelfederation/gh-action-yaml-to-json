...
var require_index_node_cjs = __commonJS((exports2) => {
  ...
  JSONPath2.prototype._getPreferredOutput = function(ea) {
    ...
    switch (resultType) {
      case "all": {
        const path = Array.isArray(ea.path) ? ea.path : JSONPath2.toPathArray(ea.path);
  ...
  JSONPath2.prototype._trace = function(expr, val, path, parent, parentPropName, callback, hasArrExpr, literalPriority) {
    let retObj;
    if (!expr.length) {
      retObj = {
        path,
  ...
  JSONPath2.prototype._slice = function(loc, expr, val, path, parent, parentPropName, callback) {
    if (!Array.isArray(val)) {
      return void 0;
    }
  ...
  JSONPath2.prototype.vm = vm__default["default"];
  exports2.JSONPath = JSONPath2;
});
...
(async () => {
  try {
    ...
    const validate_keys = core.getMultilineInput("validate_keys", {required: false});
